...
- 本轮工作：修复继续任务失败原因过度脱敏、跨设备历史任务不可见和 UI 只按本机/全局粗分组的问题。
- 当前阶段判断：该问题挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于用户明确反馈的发布候选 UX 与多设备恢复阻塞修复，影响 R06 继续备份、R07 校对、R09 恢复和 R13 云同步真实性审计。
- 本轮计划范围：改进 pywebview operation 错误 DTO，在不泄露本地路径、远端路径、token 或密码的前提下展示失败阶段、错误类型、HTTP/业务错误码和可读原因；云端 `/v1/backups` 新增 `device_id=all` 受控拉取已绑定百度账号可见设备的历史索引，客户端默认拉取全设备历史并兼容旧服务回退；备份页和工作台按本机任务与各设备任务分组展示，其他设备任务只读但可用于本机恢复/校对入口；补齐契约文档、UI 文档、spec 与自动化测试。
- 本轮完成后继续 P3 阶段 14 干净 Windows R04-R14 真实启动、授权、备份、校对、清理、恢复、断网、升级/卸载和云同步真实性复验。

## 本次验收标准

- 继续任务失败后，UI 可看到失败发生阶段、错误类型、HTTP/业务错误码、脱敏后的具体原因和下一步建议，不再只显示“详细信息已脱敏”。
- operation DTO 和任务 DTO 不输出 Device Token、百度 token、授权密码、归档密码、wrapping key、完整本地路径或完整远端路径。
- 真实云端新服务支持 `GET /v1/backups?device_id=all&limit=...`，返回当前设备及与当前设备共享百度账号绑定的其他设备历史；旧服务返回 `forbidden_device` 时客户端自动回退 `current`。
- 备份页拆分为“本机任务”和“其他设备任务”，其他设备任务按设备摘要分组，只读展示状态、阶段、来源数、同步状态和失败原因，不显示继续/暂停/取消。
- 工作台最近任务区显示本机任务和各设备任务摘要，用户能识别其他设备备份可进入恢复/校对流程。
- bridge、前端静态测试、云端 API 测试、客户端 API/历史同步测试、compileall、Go 测试和 `git diff --check` 通过。
