...
- 本轮工作：修复发布候选 UI 中继续任务、最近操作、恢复/清理/校对候选选择的可追踪性问题。
- 当前阶段判断：该问题挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于用户明确反馈的发布候选 UX 阻塞修复，影响 R06 继续备份、R07 校对、R08 清理和 R09 恢复。
- 本轮计划范围：扩展 pywebview operation DTO，补充任务/候选/选择数量等脱敏上下文；最近操作展示操作类型、关联任务、阶段、失败原因和 operation_id；备份页继续任务后在本页执行状态中可见；恢复、清理和校对页增加任务选择/筛选入口，并在候选表中展示任务、来源类型、文件数、阻塞原因和本地/远端归档状态。
- 本轮完成后继续 P3 阶段 14 干净 Windows R04-R14 真实启动、授权、备份、校对、清理、恢复、断网、升级/卸载和云同步真实性复验。

## 本次验收标准

- 继续任务提交后，页面可显示关联任务名、任务 ID 摘要、当前阶段、operation_id 和后续完成/失败状态，而不只显示“已提交继续任务”。
- 最近操作不再只显示“操作失败”，必须显示操作类型、关联任务或候选数量、阶段和脱敏失败原因。
- 恢复页可按任务筛选恢复候选，并展示来源类型、文件数、本地/远端归档状态、阻塞原因和选择框。
- 清理页可按任务筛选清理候选，并展示任务、来源/文件、清理状态、阻塞原因和选择框。
- 校对与同步页可从任务下拉选择校对任务并自动填入任务 ID/实体 ID，来源映射可按任务筛选。
- bridge 和前端静态测试覆盖 operation 上下文、失败原因展示和任务筛选入口；客户端定向测试、compileall 和 `git diff --check` 通过。
