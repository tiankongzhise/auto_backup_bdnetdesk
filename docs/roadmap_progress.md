...
- 本轮工作：补充完成后交付提示规则，要求每次汇报明确 Go 服务端和 client 的重新编译/部署要求。
- 当前阶段判断：该问题挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于用户明确插入的协作规则补强。
- 本轮计划范围：更新根 `AGENTS.MD` 和 `docs/current/agents.md`，明确完成后必须说明是否修改 Go 服务端、是否需要重新编译部署 Go 服务端、是否只需重新编译/打包 client；如涉及服务端配置文件或 env 环境变量变更，必须额外提示用户同步检查服务器配置和环境变量。
- 本轮完成后继续 P3 阶段 14 干净 Windows R04-R14 真实启动、授权、备份、校对、清理、恢复、断网、升级/卸载和云同步真实性复验。

## 本次验收标准

- `AGENTS.MD` 包含完成后交付提示规则，覆盖 Go 服务端、client、服务端配置文件和 env 环境变量变更提示。
- `docs/current/agents.md` 的提交规范同步包含同一规则。
- 本轮为文档规则变更，不涉及 Go 服务端或 client 代码编译。
- `git diff --check` 通过。
