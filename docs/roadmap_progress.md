...
- 本轮工作：P3 阶段 14 pywebview 本机设备摘要展示与百度授权密码验证入口修复，挂靠干净 Windows R04-R14 发布候选验收，不改变主排期。
- 用户反馈阻塞：客户端百度授权页显示“本机摘要 16da66855e”，该值与云端任何设备 ID 对不上；经核对该值是旧本机 `device_id` 的 SHA256 前缀，不是设备 ID 本身。同时百度授权页缺少清晰可见的授权密码验证输入区域。
- 本轮计划修复当前 UI 误导：本机设备摘要改为与云端账号表同口径的 `device_id` 前后缀摘要，避免把 `device_id` 二次哈希成不可对照摘要；百度授权页新增独立的“验证授权密码”区域，用户可选择账号、输入授权密码并执行本地解密验证，不再只依赖账号表行内 prompt。
- 本轮完成后继续回到 P3 阶段 14 干净 Windows R04-R14 发布候选验收。

## 本次验收标准

- 本机设备摘要必须由真实 `device_id` 前后缀生成，例如 `dev_...5d10`，不得再显示 `sha256(device_id)` 前缀这类无法与云端设备列表对照的摘要。
- 百度授权页必须提供独立可见的授权密码验证输入区，支持选择账号、输入授权密码、调用 `verify_baidu_token` 并展示解密验证结果。
- 自动化测试覆盖本机摘要不再二次哈希、百度授权页存在验证区且不使用 `window.prompt` 收集授权密码；客户端定向 pytest、compileall 和 `git diff --check` 必须通过。
