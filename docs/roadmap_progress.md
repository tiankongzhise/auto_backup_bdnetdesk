...
- 本轮插队工作：P3 阶段 14 普通用户使用说明文档补齐已完成，挂靠打包发布与最终验收，不改变主排期。
- 本轮已新增面向不会编程用户的项目使用说明文档，覆盖发布包获取/启动、首次配置、百度授权、备份、查看校对、原始数据清理、恢复、日常维护、故障处理、安全边界和当前发布状态。
- 本轮已同步 README 文档入口，让普通用户能从仓库首页进入使用说明；同时保留 P3-14 尚未完成干净 Windows 最终验收的真实状态，不把发布候选写成最终可交付。
- 本轮文档变更未修改客户端运行逻辑、未接触真实百度远端对象、未执行真实上传或删除；完成后回到干净 Windows 发布候选端到端验收和安装/升级/卸载验证。

## 本次验收标准

- 新增 `docs/user_guide.md`，面向普通 Windows 用户逐步说明如何准备、启动、配置、授权、备份、校对、清理、恢复和维护本项目。
- 文档必须明确当前 P3-14 状态：已有 onedir 发布包构建骨架，但最终干净 Windows 安装、授权、备份、校对、清理、恢复、升级/卸载和敏感信息审计仍待完成；用户不应把当前仓库源码状态误认为正式最终版。
- 文档必须写清楚安全边界：百度账号密码只在百度官方页面输入，授权密码/归档密码不要写入仓库或聊天，原始数据清理默认回收站，永久删除是高级危险选项，恢复默认保留两者不覆盖。
- README 必须新增普通用户说明入口。
- 文档格式检查 `git diff --check` 通过；如只改文档，不要求重新运行客户端或 Go 自动化测试。
