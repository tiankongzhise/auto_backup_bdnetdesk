...
- 本轮插队工作：P3 阶段 14 发布候选任务入口与历史同步修复，挂靠打包发布与最终验收，不改变主排期。
- 本轮计划继续修复用户反馈的发布候选阻塞：`添加来源` 必须单入口同时选择文件和文件夹并由后端自适应识别；重新编译本地 client 从云端历史同步后，`running` 任务应可继续；来源映射、原始数据清理和恢复页留空刷新应能显示本设备最近记录；普通用户主流程不得要求手动获取或输入内部任务 ID。
...
- 备份任务页只提供统一“添加来源”入口和拖拽入口，选择器可同时选择文件和文件夹，文件/文件夹类型由后端自动识别并写入任务来源。
- 从云端历史导入后仍为 `running`、`paused` 或 `failed_retryable` 的任务，在本机没有后台 worker 时必须可通过 UI 继续执行；`开始` 只用于新建待开始任务。
- 来源映射、原始数据清理和恢复页刷新前复用同一设备历史同步 helper 拉取本机云端历史；云端拉取失败时继续显示本地记录并给出脱敏状态提示。
- 来源映射、原始数据清理和恢复页默认按“全部最近记录”展示，任务筛选使用任务名/状态/更新时间/来源数，不要求用户输入内部 `backup_job_id`。
