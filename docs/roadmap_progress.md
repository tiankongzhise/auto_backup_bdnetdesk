...
- 本轮工作：沉淀 hash、文件夹 hash、去重索引和跨设备重复上传行为 code review 结论，形成专题审计文档。
- 当前阶段判断：该工作挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于用户明确要求的代码审计文档化，影响 R06 真实备份性能、R09 恢复可靠性、R13 跨设备云同步真实性审计和后续 hash/去重优化排期。
- 本轮计划范围：新增 `docs/current/hash_dedupe_review.md`，详细记录文件 hash、文件夹 hash、SQLite/outbox 重扫写入、非 payload 引用复查、跨设备历史导入和云端候选查询的现状、问题原因、边界情况、可能后果和优化方案；在当前文档入口中增加专题审计入口；更新本文件的排期变更、完成记录和后续待办。
- 本轮完成后继续 P3 阶段 14 干净 Windows R04-R14 真实启动、授权、备份、校对、清理、恢复、断网、升级/卸载和云同步真实性复验。

## 本次验收标准

- 新增审计文档能回答：当前是否检查 hash、文件夹大量小文件是否有瓶颈、另一台设备上传后本机会不会重复上传。
- 文档逐项说明问题原因、边界情况、可能后果、优化方案、优先级和需要补充的验证用例。
- 文档引用具体代码位置或模块，便于后续开发直接定位。
- 本次为文档-only 变更，不改客户端或 Go 服务端业务代码；`git diff --check` 通过。
...
### 2026-06-26：P3 阶段 14 Hash 与跨设备去重 code review 文档化

变更原因：用户要求将本轮 code review 中发现的文件 hash、文件夹 hash、大量小文件性能瓶颈、非 payload 引用复查和跨设备重复上传问题写成更详细的文档，说明原因、边界情况、可能后果和优化方案。

影响阶段：挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于发布候选前的数据一致性、恢复可靠性和跨设备云同步真实性审计补强，影响 R06 真实备份性能、R09 恢复可靠性和 R13 云同步真实性复验。

验收标准：新增 `docs/current/hash_dedupe_review.md`；文档覆盖当前 hash 检查链路、文件夹 hash 与大量小文件瓶颈、非 payload 引用复查缺口、SQLite/outbox 重扫压力、跨设备历史导入和云端候选查询边界；README、Audit、DCA 和本进度记录同步入口；`git diff --check` 通过。

回到主排期条件：本轮专题审计文档、入口索引、DCA、进度记录和提交完成后，继续进入 P3 阶段 14 干净 Windows R04-R14 发布候选验收；后续如实现优化，需要另起正式开发小项。

### 2026-06-22：P3-14 发布候选任务可追踪性与候选选择修复

变更原因：用户反馈三项发布候选 UI 阻塞：继续任务后只提示“已提交继续任务”但不知道任务在哪里；最近操作只看到“操作失败”但不知道是什么任务、失败原因；恢复/清理/校对页不知道从哪个任务恢复或清理，也不知道恢复哪个文件或文件夹，无法明确选择。

影响阶段：挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于 R06 继续备份、R07 校对、R08 清理、R09 恢复和 R10 断网补偿前必须修复的发布候选体验问题。

验收标准：operation DTO 和前端操作卡片提供脱敏上下文、任务名、阶段、失败原因和 operation_id；备份页继续任务后本页执行状态可见；恢复/清理/校对页提供任务筛选和候选明细选择；相关 bridge、webui 静态测试、compileall 和 `git diff --check` 通过。

回到主排期条件：本轮 UI/bridge 可追踪性修复、文档审计更新、测试和提交完成后，继续进入 P3 阶段 14 干净 Windows R04-R14 发布候选验收。

### 2026-06-26：P3 阶段 14 启动任务前 GitHub 远端检查门禁补强

变更原因：用户要求新增协作约束，启动任务时需要先检查 GitHub 远端是否有新提交；如果远端有提交，必须让用户显示选择是否同步远端最新代码，不能静默更新后继续任务。

影响阶段：挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于协作流程门禁补强，影响后续每次新任务启动前的读取、确认和修改顺序。

验收标准：根 `AGENTS.MD` 与 `docs/current/agents.md` 均新增任务启动前 GitHub 远端检查门禁；`docs/roadmap_progress.md` 记录该临时需求的当前工作项、计划范围、验收标准和完成记录；`git diff --check` 通过。

回到主排期条件：本轮协作规则补强、文档审计和提交完成后，继续回到 P3 阶段 14 干净 Windows R04-R14 发布候选验收。

### 2026-06-26：P3 阶段 14 继续失败原因与跨设备任务历史修复

变更原因：用户反馈继续任务失败后 UI 只显示笼统脱敏信息，看不到具体失败原因；同时 UI 实际只能看到本机任务，无法按设备区分历史，也无法把其他设备备份同步到本机用于恢复/校对。

影响阶段：挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于发布候选 UX、多设备历史同步和恢复可用性阻塞修复，影响 R06 继续备份、R07 校对、R09 恢复和 R13 云同步真实性审计。

验收标准：继续任务失败后 operation DTO 和 UI 展示失败阶段、错误类型、错误码、HTTP 状态、脱敏后的具体原因和下一步建议；云端 `/v1/backups?device_id=all` 只返回当前设备及共享百度账号绑定设备的历史；客户端默认拉取跨设备历史并兼容旧服务回退本机历史；备份页和工作台按本机任务与其他设备任务展示，其他设备任务只读但可用于恢复/校对；客户端全量 pytest、compileall、Go 测试和 `git diff --check` 通过。

回到主排期条件：本轮跨设备历史和失败原因展示修复、文档审计、测试和提交完成后，继续进入 P3 阶段 14 干净 Windows R04-R14 发布候选验收。

### 2026-06-22：P3-14 完成后交付提示规则补强

变更原因：用户要求后续每次完成修改后，必须告诉用户是否修改了 Go 服务端，是否需要重新编译并部署 Go 服务端，还是只需要重新编译 client；如涉及 Go 服务端配置文件和 env 环境变更，还需要额外提示。

影响阶段：挂靠 P3 阶段 14 打包发布与最终验收，不改变主排期；属于协作规则补强和发布候选阶段交付提示规范。

验收标准：根 `AGENTS.MD` 和 `docs/current/agents.md` 均记录完成后交付提示规则；规则覆盖 Go 服务端代码/迁移/API/schema/部署脚本、client 代码、文档-only 变更以及服务端配置/env 环境变量变更；`git diff --check` 通过。

回到主排期条件：本轮文档规则提交完成后，继续进入 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
...
### P3 阶段 14 打包发布与最终验收：Hash 与跨设备去重审计文档

- P3 阶段 14 Hash 与跨设备去重审计文档化完成；P3 阶段 14 仍在进行中，下一个开发阶段为 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- 新增 `docs/current/hash_dedupe_review.md`，回答当前项目是否检查 hash、大量小文件在文件夹 hash/扫描链路中是否会形成瓶颈、另一台设备上传后本机是否仍可能重复上传。
- 文档按问题逐项记录原因、边界情况、可能后果、优化方案、优先级和建议补充的测试，覆盖非 payload 引用复查、小文件重复读取、文件夹 hash 放大、atime 抖动、SQLite/outbox 重扫压力、跨设备去重强度和 manifest-only 恢复引用。
- `docs/current/README.md` 增加专题审计入口和阅读路径，后续修改扫描、hash、去重、manifest-only 归档、跨设备重复上传或恢复可用性前需要先读该文档。
- `docs/current/audit.md` 和 `docs/current/document_change_audit.md` 已记录本次专题审计结论和 DCA-019，防止后续只依据旧“完整 SHA256+大小去重”口径忽略性能与恢复边界。
- 本轮只修改文档和进度记录，不涉及 Go 云端服务、Go 迁移、PostgreSQL schema、客户端业务代码或前端静态资源。
- 已验证本轮文档无尾随空白，`git diff --check` 通过。

提交摘要：本次提交沉淀 hash 与跨设备去重专题 code review，新增审计文档详细说明当前 hash 检查链路、已发现风险、边界情况、可能后果和优化优先级，并在当前文档入口、总审计、DCA 和进度记录中建立后续修复入口。

后续待办：

- 若进入代码优化，优先处理 P0：打包前复查所有 content reference，manifest-only/reference archive 必须能证明 payload 可恢复。
- P1 优化建议包括备份前显式刷新跨设备历史、小文件单次读取 hash、重扫 diff 写入避免未变化内容制造 outbox 修订。
- P2 优化建议包括移除或分离 atime/mtime/ctime 语义、引入 Merkle 式文件夹 hash 和服务端 content availability 查询。

### P3 阶段 14 打包发布与最终验收：继续失败原因与跨设备任务历史修复

- P3 阶段 14 继续失败原因与跨设备任务历史修复完成；P3 阶段 14 仍在进行中，下一个开发阶段为 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- pywebview bridge 的错误 DTO 改为保留失败阶段、错误类型、业务错误码、HTTP 状态、脱敏后的具体原因和下一步建议；敏感片段按 token、密码、wrapping key、本地路径和百度远端路径局部替换，不再整句吞掉失败原因。
- 任务 DTO 新增 `device_group_label`，`last_error` 使用同一脱敏策略；备份页拆分本机任务和其他设备任务，其他设备任务按设备摘要分组，只读展示且提示可用于恢复/校对。
- Go 云端 `/v1/backups` 新增 `device_id=all` 查询口径：返回当前设备和与当前设备共享百度账号绑定的设备历史；显式任意其他设备 ID 仍返回 `forbidden_device`，避免枚举越权。
- Python 客户端 `BaiduCloudClient.list_backup_history()` 默认请求 `device_id=all`，旧服务返回 `403 forbidden_device` 时自动回退 `current`，保持部署过渡兼容。
- 文档已更新 API/数据库契约、pywebview 前端规格、设计文档、spec、DCA 和本进度记录。
- 已验证 `node --check` 覆盖 `jobs.js`、`dashboard.js` 和 `render.js`。
- 已验证客户端定向 pytest 50 项通过：`test_webview_bridge.py`、`test_webui_static.py`、`test_baidu_cloud_api.py`、`test_backup_history_sync.py`。
- 已验证客户端全量 pytest 212 项通过。
- 已验证 `uv run python -m compileall src tests` 通过。
- 已验证 `cloud-api/` 下 `go test -p=1 ./...` 通过。
- 已验证 `git diff --check` 通过。

提交摘要：本次提交修复继续任务失败原因展示过度脱敏的问题，并打通同一百度账号下跨设备备份历史同步入口；云端新增受控 `device_id=all` 历史查询，客户端默认导入跨设备索引，pywebview UI 按本机和其他设备分组展示任务，其他设备任务保持只读但可进入恢复和校对流程。

后续待办：

- 部署新版 Go 服务端后，用真实同百度账号多设备验证 `/v1/backups?device_id=all` 只返回可见设备历史。
- 在干净 Windows R06/R09/R13 继续复验失败原因可读、其他设备备份可同步到本机并用于恢复/校对。
- 完成本轮后回到 P3 阶段 14 干净 Windows R04-R14 发布候选验收。

### P3 阶段 14 打包发布与最终验收：启动任务前 GitHub 远端检查门禁补强

- P3 阶段 14 启动任务前 GitHub 远端检查门禁补强完成；P3 阶段 14 仍在进行中，下一个开发阶段为 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- 根 `AGENTS.MD` 与 `docs/current/agents.md` 新增约束：开始新任务、进入新开发小项或准备修改前，必须先检查 GitHub 远端是否有新提交；如远端领先、分叉或状态不明，必须先向用户展示是否同步远端最新代码的选择，未经确认不得静默拉取、合并、变基或重置。
- `docs/roadmap_progress.md` 当前工作项、计划范围、验收标准和排期变更记录已同步补入该临时协作门禁。
- 已验证 `git diff --check` 通过。
- 本轮只修改协作规则文档和进度记录，不涉及 Go 云端服务、Go 迁移、PostgreSQL schema 或 client 代码。

提交摘要：本次提交补强任务启动门禁，要求每次开始新任务前先检查 GitHub 远端是否有新提交，并在远端领先或分叉时先让用户显式选择是否同步远端最新代码，避免在不知情情况下继续基于过时本地代码推进。

后续待办：

- 后续启动新任务时默认先做远端检查，再决定是否同步。
- 回到 P3 阶段 14 主线。

### P3 阶段 14 打包发布与最终验收：任务可追踪性与候选选择修复

- P3 阶段 14 发布候选任务可追踪性与候选选择修复完成；P3 阶段 14 仍在进行中，下一个开发阶段为 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- bridge `OperationHandle` DTO 新增 `operation_id_hint`、`kind_label` 和 `context`，用于展示操作类型、关联任务、任务 ID 摘要、候选数量、来源显示名和目标摘要。
- `start_job`、`apply_restore`、`apply_cleanup`、`run_remote_reconcile`、`apply_remote_repairs` 在创建 operation 时填充脱敏上下文；失败时前端可看到具体任务/候选范围、阶段和脱敏失败原因，不再只显示“操作失败”。
- bridge 新增 `list_job_choices()`，复用任务 DTO 的脱敏字段，为恢复、清理、校对页提供任务下拉数据；任务 DTO 同步新增 `job_id_hint` 和 `entity_id`，便于 Cloud Sync 回读自动填充。
- 备份页继续任务后会在本页“执行状态”中立即显示关联任务、阶段、operation 编号和后续完成/失败结果；状态卡片按最新创建时间选择同类 operation，避免旧操作覆盖新继续任务。
- 工作台“最近操作”改为展示最近多条 operation，并显示操作类型、目标、任务名、阶段、失败原因和 operation 编号。
- 恢复页新增任务下拉和关键字筛选，候选表展示任务名、来源类型、文件数、本地归档可用性、远端归档状态、阻塞原因和路径摘要。
- 清理页新增任务下拉和关键字筛选，候选表展示任务名、来源/文件、清理状态、待同步提示、归档状态、阻塞原因和路径摘要。
- 校对与同步页新增任务下拉，选择任务后自动过滤来源映射、远端校对默认使用任务 ID、Cloud Sync 回读默认填入该任务 `entity_id`；上传会话和远端目录仍保留为排障入口。
- 更新 `docs/current/api_database_contract.md`、`docs/current/design.md`、`docs/current/spec.md`、`docs/current/document_change_audit.md` 和 `client/docs/frontend_spec_pywebview.md`，固化 operation 上下文、任务选择入口和候选表展示口径。
- 新增和扩展 `tests/test_webview_bridge.py`、`tests/test_webui_static.py`，覆盖 operation 上下文、任务选择 DTO、失败原因展示、继续任务可见状态和恢复/清理/校对任务筛选入口。
- 已验证仓库根目录 `node --check` 覆盖 `render.js`、`dashboard.js`、`jobs.js`、`restore.js`、`cleanup.js` 和 `reconcile.js` 均通过。
- 已验证 `client/` 下 `uv run python -m pytest -p no:cacheprovider --basetemp ..\.cache\pt tests/test_webview_bridge.py tests/test_webui_static.py` 通过，34 项通过。
- 已验证 `client/` 下 `uv run python -m compileall src tests` 通过。
- 已验证仓库根目录 `git diff --check` 通过。
- 本轮只修改客户端 pywebview bridge、前端静态资源、客户端测试和文档；不涉及 Go 云端服务、Go 迁移、PostgreSQL schema 或服务端部署脚本。

提交摘要：本次提交修复发布候选 UI 的任务可追踪性和候选选择问题，operation DTO 增加脱敏上下文并在工作台、备份、恢复、清理、校对页显示任务/候选范围、阶段和失败原因；恢复、清理、校对页新增任务下拉筛选，用户可以明确选择从哪个任务恢复、清理或校对，继续任务提交后也能在当前页看到具体执行状态。

后续待办：

- 回到 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- R06 重点验证失败/暂停/运行中任务继续后，当前页操作状态和工作台最近操作都能显示任务名、阶段、失败原因或完成摘要。
- R07/R08/R09 重点验证校对、清理、恢复均可从任务下拉选择，并能看清候选阻塞原因、本地/远端归档状态和待同步提示。

### P3 阶段 14 打包发布与最终验收：完成后交付提示规则补强

- P3 阶段 14 完成后交付提示规则补强完成；P3 阶段 14 仍在进行中，下一个开发阶段为 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
- 根 `AGENTS.MD` 新增“完成后交付提示规则”，要求每次完成修改并向用户汇报时明确说明是否涉及 Go 云端服务、是否需要重新编译并部署 Go 服务端，还是只需要重新编译/打包 client。
- 规则明确：修改 `cloud-api/`、Go 迁移、云端 HTTP API、PostgreSQL schema、服务端启动/部署脚本或 Go 依赖时，必须提示 Go 服务端重新编译，并按实际情况说明是否需要部署到服务器。
- 规则明确：涉及 Go 服务端配置文件、`.env.example`、环境变量读取逻辑、部署文档中的环境变量或启动参数时，必须额外提示同步检查服务器配置文件和 env 环境变量。
- 规则明确：只改 `client/`、客户端文档、前端静态资源、客户端测试或 Python 依赖时，必须说明不需要重新编译/部署 Go 服务端，只需要重新编译/打包 client；文档-only 规则变更则说明 Go 服务端和 client 都无需重新编译。
- `docs/current/agents.md` 的提交规范同步新增同一收尾汇报要求，避免根规则和当前开发规范漂移。
- 已验证仓库根目录 `git diff --check` 通过。
- 本轮实际只修改文档规则和进度记录，不涉及 Go 服务端代码、Go 服务端配置/env 变更，也不涉及 client 运行代码。

提交摘要：本次提交补强项目协作规则，要求后续每次完成修改后的用户汇报都必须明确 Go 服务端与 client 的重新编译/部署要求，并在涉及 Go 服务端配置文件、启动参数或 env 环境变量变更时额外提示服务器配置同步检查。

后续待办：

- 后续所有最终回复都按该规则明确写出 Go 服务端是否需要重新编译/部署、client 是否需要重新编译/打包，以及是否存在服务端配置/env 额外提示。
- 回到 P3 阶段 14 干净 Windows R04-R14 发布候选验收。
