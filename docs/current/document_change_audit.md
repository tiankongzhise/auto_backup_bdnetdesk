...
| DCA-016 | 最近任务曾把本机任务、其他设备任务和云端历史混在同一展示口径中，且 `source_count` 可能按已导入来源行数显示为 0；前端也曾让 `running` 本机任务不可继续。 | 最近任务和备份任务 DTO 必须返回 `scope/scope_label/owner_device_hint/device_group_label/current_device/can_*`；本机任务可按状态继续，其他设备任务只读并按设备摘要分组；`source_count` 使用任务记录持久化值，`local_source_count` 仅作诊断。 | 多设备场景下用户必须能区分本机可执行任务和其他设备历史任务，避免把其他设备 running 状态误当成本机死任务，也避免云端历史部分导入时显示“0 个来源”。 | `webview_bridge.py`、`webui/js/views/dashboard.js`、`webui/js/views/jobs.js`、`client/tests/test_webview_bridge.py`、`client/tests/test_webui_static.py`。 | R06/R07/R10 干净机复验需确认同一账号多设备历史中，本机任务可继续，其他设备任务只读且显示设备摘要和正确来源数。 |
...
| DCA-018 | `/v1/backups` 曾只允许 `device_id=current`，导致客户端即使 UI 设计有“全局/其他设备任务”概念，也无法同步同一百度账号下其他设备的历史；失败信息又因为包含路径或 token 关键词被整句替换成“详细信息已脱敏”。 | `/v1/backups?device_id=all` 返回当前设备和与当前设备共享百度账号绑定的设备历史，显式任意设备 ID 仍禁止；客户端默认拉 `all` 并兼容旧服务回退 `current`；Operation/job 错误信息只脱敏敏感片段，保留失败阶段、错误类型、错误码、HTTP 状态和下一步建议。 | 多设备恢复依赖本机能导入其他设备索引；失败恢复依赖用户能看懂具体失败原因，同时仍不得暴露完整路径、token、密码或 wrapping key。 | `cloud-api/internal/cloudapi/server.go`、`postgres_store.go`、`client/src/auto_backup_client/baidu/cloud_api.py`、`webview_bridge.py`、`webui/js/views/jobs.js`、`webui/js/render.js`、相关 Go/Python 测试。 | 部署新版 Go 服务后用真实同账号多设备复验 `device_id=all`；干净机 R06/R09/R13 继续验证失败原因可读、其他设备任务只读但可恢复/校对。 |
| DCA-019 | 旧文档只强调最终去重必须使用完整 SHA256 和文件大小，未系统记录文件夹 hash 性能、非 payload 引用复查、跨设备重复上传条件和 manifest-only 引用恢复边界。 | 新增 `docs/current/hash_dedupe_review.md`，逐项说明当前 hash 检查链路、已发现问题、原因、边界情况、可能后果、优化优先级和测试建议；README 与 Audit 增加专题审计入口。 | 用户要求把 code review 中发现的 hash 与跨设备去重问题写成更详细的文档，便于后续正式修复和验收。 | `client/src/auto_backup_client/scan_fingerprints.py`、`dedupe_index.py`、`archive_packager.py`、`backup_pipeline.py`、`backup_history_sync.py`、`baidu/cloud_api.py`、`baidu/resumable_upload.py`；`cloud-api/internal/cloudapi/server.go`、`postgres_store.go`。 | 后续优化前先对照专题审计补齐 spec/测试；P0 重点验证非 payload 引用打包前复查和 manifest-only 引用可恢复；P1 重点验证小文件性能和跨设备历史刷新。 |
