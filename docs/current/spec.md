...
| 上传失败重试 UI | 进行中 | 发布候选体验 | 现有底层支持重试状态，UI 已补充失败阶段、错误类型/错误码和脱敏原因展示，仍需干净机复验。 | 干净 Windows 验收中失败可读、可继续、不误标 completed。 |
...
| 来源映射 UI | 进行中 | 设备历史、SQLite、Cloud Sync API | 普通用户不需要输入内部 job_id；客户端默认拉取同百度账号可见的跨设备历史，旧服务回退本机历史。 | 任务列表按本机和其他设备分组；其他设备历史可进入恢复/校对候选，干净机仍需真实复验。 |
...
| 发布候选体验修复 | 进行中 | 用户反馈 | 修复入口、授权、设备 ID、历史同步、最近任务设备作用域、操作可追踪性和任务候选选择等阻塞。 | 开发机已补齐本机/其他设备任务区分、本机 running 继续入口、最近操作失败原因展示、恢复/清理/校对任务筛选入口，仍需 R04-R14 真实验收。 |
