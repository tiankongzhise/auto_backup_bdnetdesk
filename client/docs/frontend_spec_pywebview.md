...
- `get_operation(operation_id)`
  ...
  - 前端必须展示失败阶段、错误类型/错误码、脱敏后的具体失败原因和下一步建议，不能只显示“操作失败”或“详细信息已脱敏”。
...
- 最近任务表：展示本机/其他设备任务、设备摘要、来源数和状态；只有本机任务支持开始、继续、暂停和取消，其他设备任务只读但可作为恢复/校对入口。
...
- 任务表拆分本机任务和其他设备任务；其他设备任务按设备摘要分组只读展示，不显示继续、暂停、取消等可执行动作。
