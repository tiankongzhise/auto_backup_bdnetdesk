...
`CLOUD_API_DEVICE_TOKEN` 不得写入仓库文件。页面会直接调用真实云端 API 读取账号、选择账号、创建设备码授权 session、轮询状态并完成密文 token 写入。完成授权后，客户端会按 `account_id` 保存 password KDF salt/参数；后续可选中账号并点击“验证解密”，用同一授权密码重新派生 wrapping key 并验证云端密文 token 可本地解密。
